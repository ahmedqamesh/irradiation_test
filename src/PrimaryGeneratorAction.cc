...
PrimaryGeneratorAction::PrimaryGeneratorAction(DetectorConstruction* det) :
		G4VUserPrimaryGeneratorAction(), fParticleGun(0), fDetector(det), fRndmBeam(
				0.), fGunMessenger(0) {
	G4int n_particle = 1;
...
void PrimaryGeneratorAction::SetDefaultKinematic() {
	...
	G4double position =  3 * cm; //-0.5 * (fDetector->GetWorldSizeX());
...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent) {
	...
	if (fRndmBeam > 0.) {
		...
		G4double rbeam = 0.5 * (fDetector->GetDetectorSizeYZ()) * fRndmBeam;
