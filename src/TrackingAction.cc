...
void TrackingAction::PreUserTrackingAction(const G4Track* track )
{
  ...
  if (track->GetTrackID() == 1) {
 	for (G4int k=1; k<=fDetector->GetNbOfAbsor(); k++) {
  	...
  	//G4cout<<k<<"  start = "<<fXstartAbs[k]<<" end= "<<fXendAbs[k]<<"Material Thickness = "<<(fXendAbs[k]-fXstartAbs[k])<<" Charge= "<<fPrimaryCharge<<G4endl;
  										}
									}
      	  	  	  	  	  	  	  }
...
//G4cout<<"Abs"<<k<<"  GetxstartAbs = "<<fDetector->GetxstartAbs(k)<<"GetxendAbs = "<<fDetector->GetxendAbs(k)<<" & pos.x() = "<<pos.x()<<" & vert.x() = "<<vert.x()<<G4endl;
